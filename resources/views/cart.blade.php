...
<body>
    <div class="container">
        <div class="header">
            <h1>🛒 Shopping Cart</h1>
            <a href="{{ route('dashboard') }}" class="back-btn">← Continue Shopping</a>
        </div>
        ...
        @if($cart->item_count === 0)
            <div class="empty-cart">
                ...
                <a href="{{ route('dashboard') }}" class="back-btn">Browse Books</a>
            </div>
        @else
            <div class="cart-container">
                <div class="cart-items">
                    @foreach($items as $item)
                        <div class="cart-item">
                            <img src="{{ $item->book->cover_url ?? 'https://placehold.co/300x400?text=No+Cover' }}" 
                ...
                <div class="cart-summary">
                    ...
                    <div class="summary-item">
                        ...
                        <span>${{ number_format($cart->calculateTotalAmount(), 2) }}</span>
                    </div>
                     <!-- Coupon Section -->
                    <div class="coupon-section" style="margin: 20px 0; padding: 15px; background: #f8f9fa; border-radius: 8px;">
                        @if($cart->discount_code)
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                <span style="font-weight: 600; color: var(--success);">
                                    Discount Applied ({{ $cart->discount_code }}): -{{ $cart->discount_percent }}%
                                </span>
                        ...
                        @else
                            <form action="{{ route('cart.apply-coupon') }}" method="POST" style="display: flex; gap: 10px;">
                                @csrf
                    ...
                    <div class="summary-item">
                        <span>Tax</span>
                        <span>${{ number_format($cart->subtotal_after_discount * 0.08, 2) }}</span>
                    </div>
                    
                    <div class="summary-total">
                        <span>Total</span>
                        <span>${{ number_format($cart->subtotal_after_discount * 1.08, 2) }}</span>
                    </div>
